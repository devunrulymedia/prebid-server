...
import (
	"fmt"
	"github.com/prebid/prebid-server/adapters/unruly_spike"
	"net/http"
	"strings"
...
func newAdapterMap(client *http.Client, cfg *config.Configuration, infos adapters.BidderInfos) map[openrtb_ext.BidderName]adaptedBidder {
	ortbBidders := map[openrtb_ext.BidderName]adapters.Bidder{
		...
		openrtb_ext.BidderUnruly:       unruly_spike.NewUnrulyBidder(client, cfg.Adapters[string(openrtb_ext.BidderUnruly)].Endpoint),
